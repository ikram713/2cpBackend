...
// DeleiveryOffice Schema
const DeleiveryOfficeSchema = new mongoose.Schema({
   Name: { // Corrected typo: CoffeeSchopName -> DeleiveryOfficeName
        type: String,
        required: true,
        unique: true
    },
...
// DeleiveryOfficeSchema.index({ 'coordinates.latitude': 1, 'coordinates.longitude': 1 }, { unique: true });
//alow non -null values to be unique ..but null values also allowed 
DeleiveryOfficeSchema.index(
...
// Create the model
const DeleiveryOffice = mongoose.model('DeleiveryOffice', DeleiveryOfficeSchema); // Corrected typo: CoffeeSchopSchema -> DeleiveryOfficeSchema

module.exports = DeleiveryOffice;