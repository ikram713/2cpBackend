...
const GrocerieSchema = new mongoose.Schema({
    Name: { // Corrected typo: CoffeeSchopName -> GrocerieName
        type: String,
        required: true,
        unique: true
    },
...
// GrocerieSchema.index({ 'coordinates.latitude': 1, 'coordinates.longitude': 1 }, { unique: true });
//alow non -null values to be unique ..but null values also allowed 
GrocerieSchema.index(
...
// Create the model
const Groceries = mongoose.model('Groceries', GrocerieSchema); // Corrected typo: CoffeeSchopSchema -> GrocerieSchema
